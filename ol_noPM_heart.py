...
y = (y.iloc[:, 0] > 0).astype(float).values
y = y.ravel()
...
def ourLoss(output, target):
    ...
    loss = torch.dot(loss, torch.matmul(Fmat, loss))
...
print(f'Mean runs Test f1: {np.mean(f1s):.8f}')
print(f'Std deviation runs Test f1: {np.std(f1s):.8f}')
